...
#define SENSORS_ENABLE_RANGE_VL53L0X_ENHANCED  // Enhanced VL53L0X with altitude control
...
#endif
#ifdef SENSORS_ENABLE_RANGE_VL53L0X_ENHANCED
static bool isVl53l0xEnhancedPresent = false;
#endif
...
{
    // Enhanced calibration takes priority
    if (gyroCalibIsReady()) {
        return true;
    }
    // Basic calibration fallback
...
void processAccGyroMeasurements(const uint8_t *buffer)
{
    ...
    if (buffer == NULL) {
        DEBUG_PRINTE("ERROR: Null buffer in processAccGyroMeasurements!\n");
        return;
    }
    
    // Get temperature for enhanced calibration
    int16_t temperature = mpu6050GetTemperature();
    float temp_celsius = (float)temperature / 340.0f + 36.53f;
    
    // Safety check: validate temperature reading
    if (temperature == 0 || temp_celsius < -40.0f || temp_celsius > 85.0f) {
        DEBUG_PRINTW("WARNING: Invalid temperature reading: %d (%.1fC)\n", temperature, temp_celsius);
        temp_celsius = 25.0f; // Use safe default
    }
    ...
    */
    ...
    // Convert raw gyro readings to float for enhanced calibration
    gyroRawFloat.x = (float)gyroRaw.x;
    gyroRawFloat.y = (float)gyroRaw.y;
    gyroRawFloat.z = (float)gyroRaw.z;
    
    // Update enhanced gyroscope calibration system
    gyroCalibUpdate(&gyroRawFloat, temp_celsius);
    ...
    // Use enhanced calibration if ready
    if (gyroCalibIsReady()) {
        // Enhanced calibration takes priority
        Axis3f enhancedBias = gyroCalibGetBias();
        
        // Safety check: validate enhanced bias values are reasonable
        if (isfinite(enhancedBias.x) && isfinite(enhancedBias.y) && isfinite(enhancedBias.z) &&
            fabsf(enhancedBias.x) < 2000.0f && fabsf(enhancedBias.y) < 2000.0f && fabsf(enhancedBias.z) < 2000.0f) {
            gyroBias.x = enhancedBias.x;
            gyroBias.y = enhancedBias.y;
            gyroBias.z = enhancedBias.z;
            gyroBiasFound = true;
        } else {
            DEBUG_PRINTW("WARNING: Invalid enhanced gyro bias detected! Using basic calibration\n");
        }
    }
    
    // Final safety check: ensure gyro bias values are reasonable
    if (gyroBiasFound) {
        if (!isfinite(gyroBias.x) || !isfinite(gyroBias.y) || !isfinite(gyroBias.z) ||
            fabsf(gyroBias.x) > 5000.0f || fabsf(gyroBias.y) > 5000.0f || fabsf(gyroBias.z) > 5000.0f) {
            DEBUG_PRINTE("ERROR: Invalid gyro bias detected! Resetting to zero\n");
            gyroBias.x = gyroBias.y = gyroBias.z = 0.0f;
        }
    }
...
static void sensorsDeviceInit(void)
{
    ...
    if (mpu6050TestConnection() == true) {
        ...
        DEBUG_PRINTE("MPU6050 sensor not detected - continuing without it");
        DEBUG_PRINTE("Check I2C wiring: SDA=GPIO21, SCL=GPIO22");
...
// Enhanced Z-Ranger VL53L0X driver enabled for improved altitude control
// This runs in parallel with VL53L1X and provides enhanced functionality
#ifdef SENSORS_ENABLE_RANGE_VL53L0X_ENHANCED
    zRangerInit();

    if (zRangerTest() == true) {
        isVl53l0xEnhancedPresent = true;
        DEBUG_PRINTI("Enhanced VL53L0X I2C connection [OK].\n");
    } else {
        //TODO: Should sensor test fail hard if no connection
        DEBUG_PRINTW("Enhanced VL53L0X I2C connection [FAIL].\n");
    }

#endif

// Z-Ranger VL53L0X driver disabled - using enhanced version instead
// to prevent I2C conflicts and ensure flight ready state synchronization
#ifdef SENSORS_ENABLE_RANGE_VL53L0X_DISABLED
    zRangerInit();
    ...
    // Initialize enhanced gyro calibration system
    gyroCalibInit();
    
    DEBUG_PRINTI("Enhanced gyro calibration system initialized");
