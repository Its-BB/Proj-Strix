...
// #include "web_position_hold.h"  // Disabled old web position hold system
...
//  peerLocalizationInit();
  ...
  // Note: altitude logger will be initialized after sensors are ready
...
bool systemTest()
{
  ...
  pass &= buzzerTest();
  DEBUG_PRINTI("buzzerTest = %d", pass);
  pass &= altitudeLoggerTest();
  DEBUG_PRINTI("altitudeLoggerTest = %d", pass);
  return pass;
}
...
//  pass &= peerLocalizationTest();
  ...
  if(pass)
  {
    ...
    // Old web position hold system disabled - using new web server instead
    DEBUG_PRINTI("Note: Old web position hold system disabled in favor of new web server");
