...
class AIDetectionSystem:
    ...
    def load_models(self):
        ...
        try:
            ...
            # Load human and object detection model (COCO dataset includes person class)
            self.human_model = YOLO(self.config['detection']['human_model'])
            self.object_model = YOLO(self.config['detection']['object_model'])
            
            # For weapon detection, we'll use the same model but filter for weapon-like objects
            # In a production system, you'd want a specialized weapon detection model
    ...
    def detect_objects(self, frame: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            # Merge weapon detections with standard detections
    ...
    def draw_detections(self, frame: np.ndarray, detections: List[Dict]) -> np.ndarray:
        ...
        for detection in detections:
            ...
            # Choose color based on detection type and method
            if is_dangerous:
            ...
            # Prepare label with detection method info
            if self.config['display']['show_confidence']:
                label = f"{class_name}: {confidence:.2f}"
            else:
                label = class_name
            
            # Add detection method indicator for multi-angle/scale detections
            if 'multi_angle' in detection_method:
    ...
    def handle_dangerous_detection(self, detections: List[Dict]):
        ...
        if dangerous_detections and time.time() - self.last_alert_time > self.config['alerts']['alert_duration']:
            ...
            if self.config['logging']['save_detections']:
                ...
                # Note: This would need the current frame passed to save it
        ...
        try:
            while True:
                ...
                if latest_analysis:
                    ...
                    # Generate incident report for significant threats
                ...
                # Trigger alerts for dangerous detections
