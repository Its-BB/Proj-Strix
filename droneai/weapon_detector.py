...
class WeaponDetector:
    ...
    def __init__(self, config: dict, strategy_overrides: Optional[dict] = None):
        ...
        # Gun-like object patterns (objects that could be mistaken for or used to conceal guns)
        ...
        # Check if using custom model to set appropriate thresholds
        custom_model_path = 'weapon_detection_custom.pt'
        is_custom_model = os.path.exists(custom_model_path)
        
        if is_custom_model:
            # Higher thresholds for custom model to reduce false positives
            self.danger_threshold = 0.6  # Much higher threshold for custom model
            self.custom_knife_threshold = 0.7  # Very high threshold specifically for knife detection
            self.custom_pistol_threshold = 0.6  # High threshold for pistol detection
        else:
            # Lower thresholds for generic YOLO model
            self.danger_threshold = 0.2
            self.custom_knife_threshold = 0.2
            self.custom_pistol_threshold = 0.2
            
        self.gun_detection_enabled = not is_custom_model  # Disable shape detection when using custom model
        self.shape_detection_enabled = not is_custom_model  # Disable shape detection when using custom model
        self.suspicious_object_threshold = 0.5  # Higher threshold for suspicious objects
    ...
    def load_models(self):
        ...
        try:
            ...
            if os.path.exists(custom_model_path):
                # Use custom trained weapon detection model
                self.primary_model = YOLO(custom_model_path)
                self.secondary_model = YOLO(custom_model_path)  # Use same custom model
                logging.info(f"Custom weapon detection model loaded: {custom_model_path}")
                logging.info(f"Model classes: {self.primary_model.names}")
            else:
                # Fallback to generic YOLO models
                self.primary_model = YOLO('yolov8n.pt')
                self.secondary_model = YOLO('yolov8s.pt')
                logging.info("Generic YOLO models loaded (custom model not found)")
            
            logging.info("Weapon detection models loaded successfully")
            ...
            for result in results:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        # Apply class-specific thresholds for custom model
                        if self._should_accept_detection(class_name, confidence):
                            # Check if detected object could be a weapon
    ...
    def _detect_multi_angle(self, frame: np.ndarray) -> List[Dict]:
        ...
        # Enhanced transformations specifically optimized for pistols and knives
        transformations = [
            ...
            # Additional transformations for weapon-specific angles
            ...
            try:
                ...
                for detection in transformed_detections:
                    if transform_name.startswith('rotate_'):
                        ...
                        # Use existing transformation for standard transformations
                    ...
                    if original_bbox:  # Only add if transformation was successful
                        ...
                        # Less confidence reduction for weapon-specific detections
    ...
    def _detect_multi_scale(self, frame: np.ndarray) -> List[Dict]:
        ...
        # Enhanced scale factors specifically for weapon detection
        ...
        for scale in scale_factors:
            try:
                ...
                for detection in scaled_detections:
                    ...
                    # Slight confidence reduction for scaled detections
    ...
    def _transform_bbox_back(self, bbox: Tuple[int, int, int, int], transform_name: str, 
        ...
        try:
            if transform_name == 'horizontal_flip':
                ...
                # 90 degree clockwise rotation: (x, y) -> (y, height - x)
                new_x1 = y1
                ...
                # 270 degree clockwise rotation: (x, y) -> (width - y, x)
    ...
    def _transform_rotated_bbox(self, bbox: Tuple[int, int, int, int], angle: float, 
        ...
        try:
            ...
            # For arbitrary rotations, we approximate the transformation
            # This is a simplified approach - for perfect accuracy, we'd need the full rotation matrix
            center_x, center_y = original_width // 2, original_height // 2
            
            # Calculate bbox center relative to frame center
    ...
    def _should_accept_detection(self, class_name: str, confidence: float) -> bool:
        ...
        if is_custom_model:
            # Apply optimized thresholds specifically for pistols and knives
            if class_lower == 'knife':
                ...
                # For any other classes from custom model, be moderately strict
                return confidence >= 0.6
        else:
            # Enhanced thresholds for generic model weapon detection
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        if class_name == 'person':
                            ...
                            # Expand the bounding box to include areas around the person
                            h, w = frame.shape[:2]
                            expand_factor = 0.2
    ...
    def _detect_in_region(self, frame: np.ndarray, region: Tuple[int, int, int, int]) -> List[Dict]:
        ...
        try:
            # Use secondary model for higher accuracy in focused region
    ...
    def _detect_by_shape(self, frame: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            for contour in contours:
                ...
                # Check for knife-like shapes (elongated objects)
    ...
    def _calculate_weapon_score(self, class_name: str, confidence: float) -> float:
        ...
        # Check if we're using custom model (has specific weapon classes)
        custom_model_path = 'weapon_detection_custom.pt'
        is_custom_model = os.path.exists(custom_model_path)
        
        if is_custom_model:
            # Enhanced custom model scoring for pistols and knives
            if class_lower in ['knife']:
                base_score = 1.0
                # Bonus for knife detections (they're often harder to detect)
                if confidence > 0.4:
                    base_score = 1.2  # Boost confidence for good knife detections
            elif class_lower in ['pistol']:
                base_score = 1.0
                # Bonus for pistol detections at various angles
                if confidence > 0.35:
                    base_score = 1.15  # Boost confidence for good pistol detections
            else:
                base_score = 0.1
        else:
            # Enhanced generic YOLO model scoring
            if class_lower in ['knife']:
                base_score = 1.0
                # Extra bonus for knife detection in generic model
                if confidence > 0.3:
                    base_score = 1.3
                ...
                base_score = 0.3
            # Enhanced gun-like object detection
            elif any(word in class_lower for word in ['remote', 'cell phone', 'mobile phone']):
                # More sensitive to potential gun-like objects
        ...
        # Bonus for high confidence detections of ambiguous objects
    ...
    def _calculate_shape_weapon_score(self, contour, aspect_ratio: float, extent: float) -> float:
        ...
        # Long, thin objects (potential blades)
        if aspect_ratio > 4.0 or aspect_ratio < 0.25:
            score += 0.3
        
        # Objects with good extent (not too hollow)
        if 0.3 < extent < 0.8:
            score += 0.2
        
        # Check for straight edges (characteristic of manufactured weapons)
    ...
    def _detect_gun_patterns(self, frame: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            # Morphological operations to connect gun-like shapes
            ...
            for contour in contours:
                ...
                # Look for gun-like characteristics
    ...
    def _detect_suspicious_objects(self, frame: np.ndarray) -> List[Dict]:
        ...
        try:
            # Use primary model to detect everyday objects that could be suspicious
            results = self.primary_model(frame, conf=0.3)
            ...
            for result in results:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        # Check if object is in gun-like objects list
                        if class_name.lower() in [obj.lower() for obj in self.gun_like_objects]:
                            # Calculate suspicion score based on context
    ...
    def _calculate_gun_pattern_score(self, contour, aspect_ratio: float, area: float) -> float:
        ...
        # Gun-like aspect ratios (typically wider than tall or moderately rectangular)
        if 1.5 < aspect_ratio < 4.0:  # Handgun-like proportions
            score += 0.4
        elif 4.0 <= aspect_ratio < 8.0:  # Rifle-like proportions
            score += 0.5
        
        # Check for L-shaped or T-shaped patterns (characteristic of guns)
        hull = cv2.convexHull(contour)
        hull_area = cv2.contourArea(hull)
        contour_area = cv2.contourArea(contour)
        ...
        if hull_area > 0:
            solidity = float(contour_area) / hull_area
            # Guns often have distinctive shapes that aren't perfectly solid
            if 0.6 < solidity < 0.9:
                score += 0.3
        
        # Check for right angles and straight lines (manufactured appearance)
        epsilon = 0.02 * cv2.arcLength(contour, True)
        approx = cv2.approxPolyDP(contour, epsilon, True)
        
        # More vertices might indicate complex manufactured shape
        if 8 <= len(approx) <= 20:
            score += 0.2
    ...
    def _calculate_suspicion_score(self, class_name: str, confidence: float, bbox: Tuple[float, float, float, float]) -> float:
        ...
        # Different objects have different suspicion levels
        suspicion_levels = {
        ...
        # Bonus for objects detected with high confidence
        if confidence > 0.7:
    ...
    def _filter_detections(self, detections: List[Dict]) -> List[Dict]:
        ...
        # Sort by weapon score (highest first)
    ...
    def _detect_weapons_edge_enhanced(self, frame: np.ndarray) -> List[Dict]:
        ...
        try:
            # Apply edge enhancement filters specifically for weapon shapes
            ...
            for enhancement_name, enhanced_frame in enhanced_frames:
                ...
                for detection in enhanced_detections:
                    class_name = detection.get('class_name', '').lower()
                    # Only keep weapon-specific detections from edge enhancement
                    if (class_name in ['knife', 'pistol'] or 
                        ...
                        # Boost confidence for edge-enhanced weapon detections
    ...
    def _create_weapon_enhanced_frames(self, frame: np.ndarray) -> List[Tuple[str, np.ndarray]]:
        ...
        try:
            ...
            # Enhancement 1: High contrast for metallic objects (common in weapons)
            ...
            # Enhancement 2: Edge sharpening for weapon outlines
            ...
            # Enhancement 3: Gaussian blur removal (unsharp mask) for better weapon edges
            ...
            # Enhancement 4: Morphological operations for weapon shape enhancement
            ...
            # Enhancement 5: Histogram equalization for better visibility in different lighting
