...
static bool leaderFollowMode = false;

// Current active flight mode for cfclient detection
static uint8_t currentFlightMode = STABILIZE_MODE;

// Forward declarations
static void updateCurrentFlightMode(void);
uint8_t getCurrentFlightMode(void);
...
void setCommandermode(FlightMode mode){
  ...
  case POSSET_MODE:
    ...
    posHoldMode = false;
    posSetMode = true;
    leaderFollowMode = false;
    registerRequiredEstimator(kalmanEstimator); 
    break;
  case LEADER_FOLLOW_MODE:
    altHoldMode = true;
    posHoldMode = false;
    posSetMode = false;
    leaderFollowMode = true;
    registerRequiredEstimator(kalmanEstimator);
    break;
  default:
    altHoldMode = false;
  ...
  // Update current flight mode tracking for cfclient
  updateCurrentFlightMode();
  DEBUG_PRINTI("FlightMode = %d, Current = %d",mode, currentFlightMode);
#else
...
/**
 * Get current flight mode for external access
 */
uint8_t getCurrentFlightMode(void) {
    if (leaderFollowMode) return LEADER_FOLLOW_MODE;
    if (posSetMode) return POSSET_MODE;
    if (posHoldMode) return POSHOLD_MODE;
    if (altHoldMode) return ALTHOLD_MODE;
    return STABILIZE_MODE;
}

/**
 * Update current flight mode tracking
 */
static void updateCurrentFlightMode(void) {
    currentFlightMode = getCurrentFlightMode();
}
...
PARAM_ADD(PARAM_UINT8, leaderFollow, &leaderFollowMode)
PARAM_ADD(PARAM_UINT8, current, &currentFlightMode)  // Current active mode for cfclient
...
// Logging for flight modes - cfclient can monitor these
LOG_GROUP_START(flightmode)
LOG_ADD(LOG_UINT8, althold, &altHoldMode)
LOG_ADD(LOG_UINT8, poshold, &posHoldMode)
LOG_ADD(LOG_UINT8, posSet, &posSetMode)
LOG_ADD(LOG_UINT8, leaderFollow, &leaderFollowMode)
LOG_ADD(LOG_UINT8, current, &currentFlightMode)
LOG_ADD(LOG_UINT8, yawMode, &yawMode)
LOG_ADD(LOG_UINT8, stabModeRoll, &stabilizationModeRoll)
LOG_ADD(LOG_UINT8, stabModePitch, &stabilizationModePitch)
LOG_ADD(LOG_UINT8, stabModeYaw, &stabilizationModeYaw)
