 ...
 * 
 * Serves a basic HTML page with live camera stream
 */
...
// Simple HTML page - Full screen camera stream
...
"        </div>"
"        <div class='footer'>"
"            <p>Connected | WiFi: GNXS-2.4G-854460 | FPS: ~5</p>"
"        </div>"
"    </div>"
"</body>"
"</html>";
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    while (true) {
        ...
        // Frame rate control: ~5 FPS - balance between latency and stability
...
esp_err_t web_server_start(void)
{
    ...
    config.max_open_sockets = 2;  // Only allow 1-2 connections
    config.max_uri_handlers = 8;
    config.core_id = 1;  // Use second core for better performance
    config.stack_size = 16384;  // Larger stack for streaming
